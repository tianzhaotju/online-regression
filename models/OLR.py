...
class OnlineLR:
    ...
    def offline_train(self,EPOCH=1):
        ...
        for epoch in range(EPOCH):
            self.olr.fit(self.trainX, self.trainY)
            print epoch
        print "train finish!"
        self.save()
        print "save model!"
    ...
    def save(self):
        ...
        cPickle.dump(self.olr, open("./save/olr.pkl", "wb"))

    def load(self):
        # 读取Model
        self.olr = cPickle.load(open("./save/olr.pkl", "rb"))
    ...
    def test(self,write=True):
        ...
        self.init_data()
        t1 = time.time()
        preds = self.olr.predict(self.testX)
        ...
        if write:
            f = open("./result/olr_result.txt", "wb")
            errs = 0
