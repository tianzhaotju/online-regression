...
class OnlineKNN:
    ...
    def offline_train(self,EPOCH=1):
        ...
        for epoch in range(EPOCH):
            self.oknn.fit(self.trainX, self.trainY)
            print epoch
        print "train finish!"
        self.save()
        print "save model!"
    ...
    def save(self):
        ...
        cPickle.dump(self.oknn, open("./save/oknn.pkl","wb"))

    def load(self):
        # 读取Model
        self.oknn = cPickle.load(open("./save/oknn.pkl","rb"))
    ...
    def test(self,write=True):
        ...
        self.init_data()
        t1 = time.time()
        preds = self.oknn.predict(self.testX)
        ...
        if write:
            f = open("./result/oknn_result.txt","wb")
            errs = 0
