...
class OnlineSVR:
    ...
    def offline_train(self,EPOCH=1):
        ...
        for epoch in range(EPOCH):
            self.osvr.fit(self.trainX, self.trainY)
            print epoch
        print "train finish!"
        self.save()
        print "save model!"
    ...
    def save(self):
        ...
        cPickle.dump(self.osvr, open("./save/osvr.pkl","wb"))

    def load(self):
        # 读取Model
        self.osvr = cPickle.load(open("./save/osvr.pkl","rb"))
    ...
    def test(self,write=True):
        ...
        self.init_data()
        t1 = time.time()
        preds = self.osvr.predict(self.testX)
        ...
        if write:
            f = open("./result/osvr_result.txt","wb")
            errs = 0
