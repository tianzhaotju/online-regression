...
class OnlineMLP:
    ...
    def __init__(self,input,dataPath='./data/offline/',pct=0.9):
        ...
        self.init_data()
        # self.omlp = MLPRegressor(hidden_layer_sizes=(64,64,1),  activation='relu',max_iter=500)
        self.omlp = MLPRegressor(hidden_layer_sizes=(256,256,))
    ...
    def offline_train(self,EPOCH=1):
        ...
        for epoch in range(EPOCH):
            self.omlp.fit(self.trainX, self.trainY)
            print epoch
        print "train finish!"
        self.save()
        print "save model!"
    ...
    def save(self):
        ...
        cPickle.dump(self.omlp, open("./save/omlp.pkl","wb"))

    def load(self):
        # 读取Model
        self.omlp = cPickle.load(open("./save/omlp.pkl","rb"))
    ...
    def test(self,write=True):
        ...
        self.init_data()
        t1 = time.time()
        preds = self.omlp.predict(self.testX)
        ...
        if write:
            f = open("./result/omlp_result.txt","wb")
            errs = 0
